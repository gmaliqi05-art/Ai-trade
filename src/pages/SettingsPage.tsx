...
export default function SettingsPage() {
  ...
  const changePw = async () => {
    if (pwForm.new !== pwForm.confirm) { setPwMsg('Fjalëkalimet nuk përputhen.'); return; }
    if (pwForm.new.length < 6) { setPwMsg('Minimum 6 karaktere.'); return; }
    setSaving(true);
    const { error } = await supabase.auth.updateUser({ password: pwForm.new });
    setPwMsg(error ? error.message : 'Fjalëkalimi u ndryshua!');
  ...
  const sections = [
    ...
    { id: 'security' as Section, label: 'Siguria', icon: Shield },
    { id: 'notifications' as Section, label: 'Njoftimet', icon: Bell },
    { id: 'subscription' as Section, label: 'Abonimi', icon: CreditCard },
  ];

  const tiers = [
    { id: 'free', name: 'Falas', price: '$0', features: ['10 aktive të ndjekur', 'Sinjale bazë', '3 alarme çmimi', 'Mbështetje komuniteti'], border: 'border-gray-700', badge: '' },
    { id: 'pro', name: 'Pro', price: '$29', features: ['Aktive të pakufizuara', 'Të gjitha sinjalet AI', 'Alarme të pakufizuara', 'Mbështetje prioritare', 'Analitikë e avancuar'], border: 'border-amber-500', badge: 'Më i popullarizuari' },
    { id: 'elite', name: 'Elite', price: '$79', features: ['Gjithçka e Pro-s', 'Menaxher portofoli AI', 'Strategji të personalizuara', 'Këshilltar i dedikuar', 'Qasje API'], border: 'border-gray-600', badge: '' },
  ];
  ...
  return (
    <div className="p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div><h2 className="text-2xl font-bold text-white">Cilësimet</h2><p className="text-gray-400 text-sm">Menaxho preferencat e llogarisë tënde</p></div>
      </div>
        ...
        <div className="lg:w-56 flex-shrink-0">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-2">
            ...
            <div className="mt-2 pt-2 border-t border-gray-800">
              <button onClick={signOut} className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm text-gray-400 hover:bg-red-900/30 hover:text-red-400 transition-all"><LogOut className="w-4 h-4" />Dil</button>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1">
          {activeSection === 'profile' && (
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
              <h3 className="text-white font-semibold mb-5 flex items-center gap-2"><User className="w-4 h-4 text-amber-400" />Të dhënat e profilit</h3>
              <div className="flex items-center gap-4 mb-6 p-4 bg-gray-800/50 rounded-2xl border border-gray-700/50">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1 mt-1"><Crown className="w-3 h-3 text-amber-400" /><span className="text-amber-400 text-xs font-medium capitalize">Plani {profile?.subscription_tier || 'free'}</span></div>
                </div>
              </div>
              <div className="space-y-4">
                {[{ label: 'Emri i plotë', key: 'full_name' as const, type: 'text' }, { label: 'Username', key: 'username' as const, type: 'text' }].map(f => (
                  <div key={f.key}>
          ...
          {activeSection === 'security' && (
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
              <h3 className="text-white font-semibold mb-5 flex items-center gap-2"><Shield className="w-4 h-4 text-amber-400" />Cilësimet e sigurisë</h3>
              <div className="space-y-4">
                {[{ label: 'Fjalëkalimi i ri', key: 'new' as const }, { label: 'Konfirmo fjalëkalimin', key: 'confirm' as const }].map(f => (
                  <div key={f.key}>
                    <label className="block text-sm font-medium text-gray-300 mb-1.5">{f.label}</label>
                ...
                <button onClick={changePw} disabled={saving} className="flex items-center gap-2 bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-gray-950 font-semibold px-5 py-2.5 rounded-xl text-sm transition-all">
                  {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Shield className="w-4 h-4" />}Ndrysho fjalëkalimin
                </button>
              </div>
            </div>
          )}
          ...
          {activeSection === 'notifications' && (
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-white font-semibold flex items-center gap-2"><Bell className="w-4 h-4 text-amber-400" />Preferencat e njoftimeve</h3>
                {notifSaved && <span className="text-green-400 text-xs flex items-center gap-1"><Check className="w-3 h-3" />U ruajt</span>}
              </div>
              <div className="space-y-3">
                {[
                  { key: 'signals' as const, label: 'Sinjale AI', desc: 'Njoftohu kur gjenerohen sinjale të reja tregtimi' },
                  { key: 'priceAlerts' as const, label: 'Alarme çmimi', desc: 'Njoftime kur aktivizohen alarmet e tua të çmimit' },
                  { key: 'tradeConfirmations' as const, label: 'Konfirmime tregtie', desc: 'Konfirmim pas çdo tregtie të ekzekutuar' },
                  { key: 'newsletter' as const, label: 'Buletini i tregut', desc: 'Përmbledhje javore e tregut dhe analizat' },
                ].map((item) => (
